...
class Aurora_Vision_Sites:
    def __init__(self, username, password, site_name):
        self.username = username
        self.password = password
        self.site_name = site_name
        self.chrome_driver_path = "/usr/bin/chromedriver"
        self.login_url = "https://www.auroravision.net/ums/v1/loginPage?redirectUrl=https:%2F%2Fwww.auroravision.net%2Fdash%2F&cause=MISSING_TOKEN"
        ...
        self.email_subject = "error in AuroraVision " + self.site_name + " cron job"
        self.login_errors = 0
    ...
    def login(self):
        try:
            ...
            time.sleep(2)
            uname = self.driver.find_element_by_name("userId")
            uname.send_keys(self.username)

            pword = self.driver.find_element_by_name("password")
            pword.send_keys(self.password)

            self.driver.find_element_by_xpath("//button[@class='cux-btn cux-btn-primary full-width']").click()
            time.sleep(2)
        except:
            print("Login")
            time.sleep(2)
    ...
    def get_html(self):
        try:
            ...
            self.driver.find_element_by_xpath("//div[@title='Device Status']").click()
            ...
            return soup.find_all("td", attrs={"class": "power"})
        except:
            print("Html")
            time.sleep(2)
            self.driver.quit()
    ...
    def parse_html_get_data(self, html):
        try:
            ...
            wattage = 0
            for value in html:
                if "perfBar" in str(value):
                    temp = re.split('<|>', str(value))
                    if temp[4] != "--":
                        wattage += float(temp[4]) * 1000
